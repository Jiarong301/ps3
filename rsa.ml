...
(* take_first lst n -- Returns the first n elements of list lst, or
   the whole list if it has fewer than n elements *)
...
(* div_sing b n -- Divides a bignum b (actually, just the coefficients
   part) by a single "digit" n, that is, returns a bignum representing
   b/n, where n is an integer less than cBASE, and the remainder b mod
   n *)
...
(* is_prime b -- Returns true if bignum b is prime, false otherwise,
   using the Rabin-Miller primality testing algorithm. *)
...
(* encrypt_decrypt_bignum n e_or_d s -- Encrypt or decrypt a bignum
   s. To encrypt, pass in the arguments n e s. To decrypt, pass in the
   inputs n d s. *)
let encrypt_decrypt_bignum (n : bignum) (e_or_d : bignum)
			   (s : bignum)
                         : bignum =
  exp_mod s e_or_d n ;;
...
let encrypt (n : bignum) (e : bignum) (s : string) =
  encrypt_decrypt_bignums n e (chars_to_bignums (explode s) (bytes_in_key n)) ;;
